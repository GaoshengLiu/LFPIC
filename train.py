...
class RateDistortionLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        out["bpp_loss"] = sum(
            (torch.log(likelihoods).sum() / (-math.log(2) * num_pixels))    ###?/log2
            for likelihoods in output["likelihoods"].values()
        )
        out["mse_loss"] = self.mse(output["x_hat"], target) * 255 ** 2      ###?*255
...
def configure_optimizers(net, args):
    ...
    parameters = {
        ...
        if not n.endswith(".quantiles") and p.requires_grad ##分位数；分位点 quantile
    }
    aux_parameters = {
        n
        for n, p in net.named_parameters()
...
def train_one_epoch(
    ...
    for i, (d) in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
        d = d.to(device)
        ...
        #     )
    print(f"Train epoch {epoch+1}: Average losses:"
          f"\tLoss: {train_loss.avg:.3f} |"
          f"\tMSE loss: {train_mse_loss.avg:.3f} |"
          f"\tBpp loss: {train_bpp_loss.avg:.4f} |"
          f"\tTime (s) : {time.time()-start:.4f} |"
          )
    txtfile = open('./checkpoint/training.txt', 'a')
    txtfile.write(f"Train epoch {epoch+1}: Average losses:"
          f"\tLoss: {train_loss.avg:.3f} |"
          f"\tMSE loss: {train_mse_loss.avg:.3f} |"
          f"\tBpp loss: {train_bpp_loss.avg:.4f} |"
          f"\tTime (s) : {time.time()-start:.4f}\n")
...
def test_epoch(epoch, test_dataloader, model, criterion):
    ...
    txtfile = open('./checkpoint/training.txt', 'a')
    txtfile.write(f"Test epoch {epoch+1}: Average losses:"
        f"\tLoss: {loss.avg:.3f} |"
        f"\tMSE loss: {mse_loss.avg:.3f} |"
        f"\tBpp loss: {bpp_loss.avg:.4f} |"
        f"\tAux loss: {aux_loss.avg:.4f}\n")
...
def parse_args(argv):
    ...
    parser.add_argument(
        "-d", "--dataset", type=str, default='./Data/Train', help="Training dataset") #required=True, 
    parser.add_argument(
        "-m", "--model", default="Proposed",) #Net_w_AG
    parser.add_argument(
        "-e", "--epochs", default=50, type=int, help="Number of epochs (default: %(default)s)",)
    parser.add_argument(
        "-lr", "--learning-rate", default=1e-4, type=float, help="Learning rate",)
    parser.add_argument(
        "-n",  "--num-workers", type=int, default=32, help="Dataloaders threads",)
    parser.add_argument(
        "--lambda", dest="lmbda", type=float, default=1e-3, help="Bit-rate distortion parameter",)
    parser.add_argument(
        "--batch-size", type=int, default=4, help="Batch size")
    parser.add_argument(
        "--test-batch-size", type=int, default=4, help="Test batch size",)
    parser.add_argument(
        "--aux-learning-rate", type=float, default=1e-3, help="Auxiliary loss learning rate",)
    parser.add_argument(
        "--patch-size", type=int, nargs=2, default=(384, 384), help="Size of the patches to be cropped",)
    parser.add_argument(
        "--cuda", action="store_true", default=True, help="Use cuda")
    parser.add_argument(
        "--save", action="store_true", default=True, help="Save model to disk")
    parser.add_argument(
        "--seed", type=float, default=1926, help="Set random seed for reproducibility")
    parser.add_argument(
        "--clip_max_norm", default=1.0, type=float, help="gradient clipping max norm",)
    parser.add_argument(
        "--pretrained", action="store_true", default=False, help="use the pretrain model to refine",)
    parser.add_argument('--gpu-id', default='1', type=str, help='id(s) for CUDA_VISIBLE_DEVICES')
    parser.add_argument('--savepath', default='./checkpoint/PLConvTrans01', type=str, help='Path to save the checkpoint')
...
def main(argv):
    ...
    train_dataset = ImageFolder(args.dataset, split="InSquare", transform=train_transforms)
    test_dataset = ImageFolder(args.dataset, split="FullTest", transform=test_transforms)
    ...
    # lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[35,80], gamma=0.1, last_epoch=34)
    ...
    #     last_epoch = 0 #checkpoint["epoch"] + 1#0

    best_loss = float("inf")
    lr_scheduler._step_count = last_epoch
    for epoch in range(last_epoch, args.epochs):
        ...
        if args.save:
            ...
            save_checkpoint(
                ...
                filename=os.path.join(args.savepath, "checkpoint_{}.pth.tar".format(epoch+1))
            )
            if is_best:
                DelfileList(args.savepath, "checkpoint_best")
                save_checkpoint(
                    ...
                    filename=os.path.join(args.savepath, "checkpoint_best_loss_{}.pth.tar".format(epoch+1))
                )

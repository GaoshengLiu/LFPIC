...
########original padding
    ...
    with open(csvfile, 'a+') as f:
        row = [imgpath[-1], bpp * num_pixels, num_pixels, bpp, metrics["ms-ssim"],
...
def eval_model(model, filepaths, entropy_estimation=False, half=False, outputpath='Recon', patch=576):
    ...
    with open(csvfile, 'w') as f:
        row = ['name', 'bits', 'pixels', 'bpp', 'ms-ssim', 'psnr', 'enc_time', 'dec_time']
        write = csv.writer(f)
        write.writerow(row)
    for f in filepaths:
...
def setup_args():
    ...
    # Common options.
    parser.add_argument("--dataset", type=str, default='./Data/Train/Test/', help="dataset path")
    parser.add_argument(
        "--output_path",
        default="Recon",

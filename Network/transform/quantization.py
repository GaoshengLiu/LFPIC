...
class LatentResidualPrediction1x1(nn.Module):
    def __init__(self, in_dim, out_dim, act=nn.GELU):
        super().__init__()
        diff = abs(out_dim - in_dim)
        # such setting leads to much more parameters, you'd better use the setting in Minnen'20 ICIP paper.
        self.lrp_transform = nn.Sequential(
            conv1x1(in_dim, in_dim - diff // 4),
            act(),
            conv1x1(in_dim - diff // 4, in_dim - diff // 2),
            act(),
            conv1x1(in_dim - diff // 2, in_dim - diff * 3 // 4),
            act(),
            conv1x1(in_dim - diff * 3 // 4, out_dim),
        )
